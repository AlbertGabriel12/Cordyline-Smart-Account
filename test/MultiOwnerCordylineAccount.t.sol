...
contract MultiOwnerCordylineAccountTest is Test {
    ...
    MultiOwnerCordylineAccount public account;
    MultiOwnerCordylineAccount public contractOwnedAccount;
    ...
    function setUp() public {
        ...
        MultiOwnerCordylineAccountFactory factory = new MultiOwnerCordylineAccountFactory(address(this), entryPoint);
    ...
    function testExecuteCanBeCalledByEntryPointWithContractOwnerSpecified() public {
        ...
        op.signature = abi.encodePacked(
            BaseCordylineAccount.SignatureType.CONTRACT_WITH_ADDR,
            contractOwner,
    ...
    function testRejectsUserOpWithPartialContractOwnerSpecified() public {
        ...
        op.signature = abi.encodePacked(
            BaseCordylineAccount.SignatureType.CONTRACT_WITH_ADDR, bytes10(bytes20(address(contractOwner)))
        );
        PackedUserOperation[] memory ops = new PackedUserOperation[](1);
        ops[0] = op;
    ...
    function testEntryPointCanUpdateOwners() public {
        ...
        ownersToRemove[0] = eoaAddress;
        PackedUserOperation memory op = _getSignedOp(
            abi.encodeCall(MultiOwnerCordylineAccount.updateOwners, (ownersToAdd, ownersToRemove)), EOA_PRIVATE_KEY
        );
        PackedUserOperation[] memory ops = new PackedUserOperation[](1);
        ops[0] = op;
        vm.expectEmit(true, true, false, false);
        ...
        PackedUserOperation memory op = _getSignedOp(
            abi.encodeCall(
                ...
                (
                    address(account),
                    0,
                    abi.encodeCall(MultiOwnerCordylineAccount.updateOwners, (ownersToAdd, ownersToRemove))
                )
            ),
            EOA_PRIVATE_KEY
        );
    ...
    function testCannotAddExistingOwner() public {
        ...
        vm.prank(eoaAddress);
        vm.expectRevert(abi.encodeWithSelector(MultiOwnerCordylineAccount.InvalidOwner.selector, (eoaAddress)));
        account.updateOwners(ownersToAdd, new address[](0));
    }
    ...
    function testCannotAddZeroAddressAsOwner() public {
        ...
        vm.prank(eoaAddress);
        vm.expectRevert(abi.encodeWithSelector(MultiOwnerCordylineAccount.InvalidOwner.selector, (address(0))));
        account.updateOwners(ownersToAdd, new address[](0));
    }
    ...
    function testCannotRemoveAllOwners() public {
        ...
        vm.prank(eoaAddress);
        vm.expectRevert(MultiOwnerCordylineAccount.EmptyOwnersNotAllowed.selector);
    ...
    function testCannotAddLightContractItselfAsOwner() public {
        ...
        vm.prank(eoaAddress);
        vm.expectRevert(abi.encodeWithSelector(MultiOwnerCordylineAccount.InvalidOwner.selector, (address(account))));
        account.updateOwners(ownersToAdd, new address[](0));
    }
    ...
    function testRemoveNonexistantOwner() public {
        ...
        vm.prank(eoaAddress);
        vm.expectRevert(abi.encodeWithSelector(MultiOwnerCordylineAccount.OwnerDoesNotExist.selector, (address(0x100))));
        account.updateOwners(new address[](0), ownersToRemove);
    }
    ...
    function testIsValidSignatureRejectsContractOwnerUnspecified() public {
        ...
        vm.expectRevert(abi.encodePacked(BaseCordylineAccount.InvalidSignatureType.selector));
    ...
    function _getStorage(bytes32 position)
        internal
        pure
        returns (MultiOwnerCordylineAccount.CordylineAccountStorage storage storageStruct)
    {
        assembly {
