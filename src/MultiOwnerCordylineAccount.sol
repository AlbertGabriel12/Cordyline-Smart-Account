...
/// @dev Like CordylineAccount, but multiple owners are supported. The account is initialized with a list of owners,
/// and the `updateOwners` method can be used to add or remove owners.
contract MultiOwnerCordylineAccount is BaseCordylineAccount, CustomSlotInitializable {
    ...
    event CordylineAccountInitialized(IEntryPoint indexed entryPoint, address[] owners);
    ...
    function _initialize(address[] calldata owners_) internal virtual {
        emit CordylineAccountInitialized(_ENTRY_POINT, owners_);
    ...
    {
        name = "MultiOwnerCordylineAccount";
